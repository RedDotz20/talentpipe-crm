...
const COMPANIES = [
  ...
  { name: 'Initech', slug: 'initech', adminEmail: 'admin@initech.com' },
  {
    name: 'Umbrella Corp',
    slug: 'umbrella-corp',
    adminEmail: 'admin@umbrella-corp.com',
  },
  {
    name: 'Stark Industries',
    slug: 'stark-industries',
    adminEmail: 'admin@stark-industries.com',
  },
];
...
const CANDIDATES = [
  ...
  },
  {
    ...
    firstName: 'David',
    lastName: 'Kim',
    skills: ['JavaScript', 'Vue.js', 'HTML/CSS', 'Tailwind CSS'],
  },
  {
    email: 'candidate5@test.com',
    firstName: 'Aisha',
    lastName: 'Patel',
    skills: ['Go', 'Kubernetes', 'CI/CD', 'Terraform'],
  },
  {
  ...
  },
  {
    ...
    firstName: 'Emma',
    lastName: 'Wilson',
    skills: ['SQL', 'Python', 'Data Analysis', 'Tableau'],
  },
  {
...
const JOB_POOL = [
  ...
  },
  {
    ...
    location: 'Remote',
    setup: 'work-from-home',
    skills: ['Communication', 'Agile/Scrum'],
  },
  {
  ...
  },
  {
    ...
    location: 'Manila',
    setup: 'hybrid',
    skills: ['Playwright', 'Cypress', 'Team Leadership'],
  },
  {
  ...
  },
  {
    ...
    location: 'Cebu',
    setup: 'on-site',
    skills: ['Node.js', 'REST API', 'Communication'],
  },
  {
  ...
  },
  {
    ...
    location: 'Tokyo',
    setup: 'on-site',
    skills: ['Communication', 'REST API', 'Project Management'],
  },
];
...
async function hash(val: string): Promise<string> {
  return argon2.hash(val) as Promise<string>;
}

async function provisionCompanySchema(
  client: PoolClient,
  companyId: string,
): Promise<void> {
...
async function seedSuperAdmins(client: PoolClient): Promise<void> {
...
async function seedPermissionPresets(client: PoolClient): Promise<void> {
  ...
  for (const c of CANDIDATES) {
    ...
    await client.query(
      ...
      [
        ...
        c.firstName,
        c.lastName,
        `+63 917 555 01${CANDIDATES.indexOf(c) + 1}`,
      ],
    );
...
async function seedJob(
  ...
  const createdAt = new Date(
    now.getTime() - (5 - (jobIndex % 6)) * 24 * 60 * 60 * 1000,
  );
  await client.query(
...
async function seedApplication(
  ...
  await client.query(
    ...
    [
      ...
      candidate.email,
      phone,
      JSON.stringify(appliedSkillIds),
      `I am excited to apply for the ${jobTitle} position at ${company.name}.`,
      matchScore(candidate.skills, jobSkills),
      appliedAt,
    ],
  );
  await client.query(
...
async function seedCompany(
  ...
  for (let i = 0; i < jobs.length; i++) {
    jobIds.push(
      await seedJob(client, company, companyId, jobs[i], adminId, skillIds, i),
    );
  }
  ...
  for (let i = 0; i < CANDIDATES.length; i++) {
    ...
    const appliedAt = new Date(
      now.getTime() - (29 - i * 3) * 24 * 60 * 60 * 1000,
    );
  ...
  if (done) {
    ...
    await seedInterview(
      ...
      past,
      {
        rating: 4,
        comments:
          'Strong technical skills and good culture fit. Recommend moving forward.',
      },
    );
  }
...
export async function seedDatabase(pool: Pool): Promise<void> {
  ...
  try {
    ...
    throw err; // caller (CLI or boot hook) decides exit behavior
  } finally {
    client.release();
  }
}
