...
2. **Isolation Failure Checks:** If a tenant leak or data routing error occurs, immediately check the request-scoped `AsyncLocalStorage` mapping inside the `TenantContextInterceptor` and verify if the repository properly invoked `forCurrentTenant()`[cite: 1].
