...
| TanStack Router | Routing | Type-safe, file-based routes with role-gated layout trees (`/admin/*` for SuperAdmin, `/company/*` for internal roles, pathless `_candidate` layout → `/dashboard`, `/applications`, `/bookmarks`, `/settings` for Candidate); `beforeLoad` guards redirect to the correct platform by role |
...
| PostgreSQL | Primary database | Chosen specifically for **schema-per-company isolation**. PG natively supports multiple schemas within one database — MySQL's "schema" = "database", making schema-per-company far more complex there. Uses `search_path` for per-request schema routing with a single connection pool. Additional features: `pgcrypto`, `citext`, `ROW LEVEL SECURITY` as optional defense layers |
...
| `AsyncLocalStorage` (Node built-in) | Request-scoped company context | Binds `companyId`/`userId`/`role` once per request so repositories read it internally instead of it being manually passed (and potentially forgotten) through every function call — see `05_DATA_ISOLATION_STRATEGY.md` |
...
| Playwright | (Planned) E2E test that also doubles as a demo script: sign up a company, post a job, submit an application, drag it through pipeline stages |
...
| `tsx` seed script | Generates synthetic seed data (SuperAdmin/Company/Candidate accounts) — no external dataset required |
...
2. **Caching** — dashboard aggregate queries (e.g. "applications per pipeline stage" counts for the Kanban board header) are cached with a short TTL and invalidated on write, avoiding repeated expensive `GROUP BY` queries on every page load. Cache keys are always prefixed `company:{companyId}:...` so companies never collide on the same key and one company's cache can be flushed independently — see `05_DATA_ISOLATION_STRATEGY.md`. ⬜ Planned (M6).
...
- **No real billing integration** — plan tiers are static config, not connected to a payment processor. Explainable as "out of scope for a portfolio demo, but the company/plan model is designed to support it."
