...
**Architecture:** Modules in `backend/src/modules/<name>/` (controller → service → repository), DTOs via Zod + `ZodValidationPipe`, all DB access through repositories extending `BaseRepository`. Tenant-scoped repos use `withDb('current')`, shared tables use `withDb('public')`. Frontend uses `frontend/src/api/` clients + TanStack Query hooks under the feature folder, mutations on `useApiMutation`.

**Tech Stack:** NestJS 11 + Drizzle ORM + PostgreSQL (schema-per-tenant), Zod 4, React 19 + Mantine 9 + TanStack Query 5 + TanStack Router 1.
...
- All tenant-scoped DB access via repositories extending `BaseRepository` with `withDb('current', ...)`; public/shared via `withDb('public', ...)`. No direct Drizzle client outside `repositories/`.
- Role guard: global `RolesGuard` (reads `@Roles` metadata) + route-level `@UseGuards(AuthGuard('jwt'))`. Roles: `OrgAdmin`, `Recruiter`, `HiringManager`, `Interviewer`, `SuperAdmin`, `Candidate`.
- Job postings status values: `draft` | `open` | `closed` (default `draft`). `jobListingsIndex.status` = `open` | `closed`.
- Required skills are validated against `public.skills` (application layer) then stored in the tenant-scoped `job_required_skills` join table (`jobPostingId`, `skillId`).
- `@CurrentUser()` decorator returns `TenantContext { tenantId, userId, role }`. `getTenantId()` from `common/context/tenant-context` returns the tenant id (throws if none).
...
- Internal job/candidate GET endpoints are restricted to `OrgAdmin`/`Recruiter`/`HiringManager` (NOT Candidate — candidates use `/candidate/jobs` and would break schema routing).
...
- Create: `frontend/src/features/org/job-postings/hooks/useJobPostings.ts`
- Create: `frontend/src/features/org/job-postings/hooks/useSkills.ts`
- Create: `frontend/src/features/org/candidates/hooks/useCandidates.ts`
...
- [ ] **Step 1: Extend `queryKeys.ts`** — add an `org` group:
...
- [ ] **Step 5: `features/org/job-postings/hooks/useJobPostings.ts`**
...
- [ ] **Step 6: `features/org/job-postings/hooks/useSkills.ts`**
...
- [ ] **Step 7: `features/org/candidates/hooks/useCandidates.ts`**
...
```bash
git add frontend/src/api frontend/src/features/org
...
**Files:**
- Create: `frontend/src/features/org/job-postings/JobPostingList.tsx`
- Create: `frontend/src/features/org/job-postings/JobPostingForm.tsx`
- Create: `frontend/src/features/org/job-postings/RequiredSkillsPicker.tsx`
- Create: `frontend/src/features/org/candidates/CandidateList.tsx`
- Create: `frontend/src/features/org/candidates/CandidateProfile.tsx`
- Create: `frontend/src/routes/org/job-postings.tsx`
- Create: `frontend/src/routes/org/candidates.tsx`
...
- Produces: two routable pages `/org/job-postings` and `/org/candidates` (parents already gated in `routes/org.tsx` `beforeLoad`).
...
- [ ] **Step 6: `routes/org/job-postings.tsx`**
...
- [ ] **Step 7: `routes/org/candidates.tsx`**
...
- [ ] **Step 8: Regenerate the route tree + typecheck + lint + build** — run `cd frontend && npm run build` (vite build regenerates `routeTree.gen.ts` to include the two new routes), then `npm run typecheck && npm run lint`. Expected: PASS, route tree contains `OrgJobPostingsRoute` and `OrgCandidatesRoute`.

- [ ] **Step 9: Commit**

```bash
git add frontend/src/features/org frontend/src/routes frontend/src/routeTree.gen.ts
git commit -m "feat(m2): org job postings and candidates pages"
```
