...
1. **God classes.** `AuthService` (369 lines) owns tenant provisioning, user seeding, three login flows, candidate signup, refresh rotation, logout, and three near-duplicate token generators. It does ~12 raw Drizzle calls and reads `process.env` directly.
...
3. **Services bypass repositories.** `AuthService` and `CandidateAccountService.apply()` reach straight into `DrizzleSchemaService`. `UserRepository` exists but is dead code (`forCurrentTenant()` throws outside a tenant context, which signin runs in).
...
│
├── common/
│   ├── context/tenant-context.ts    # ALS store (moved from interceptors/)
│   ├── auth/
...
│   ├── tenant.repository.ts         # standardized return shapes
│   ├── refresh-token.repository.ts  # NEW (pulled out of AuthService)
│   ├── candidate.repository.ts      # NEW (tenant-scoped candidates)
│   ├── application.repository.ts    # NEW (tenant-scoped applications)
│   ├── pipeline-stage.repository.ts # NEW (tenant-scoped pipeline_stages)
...
│   │   └── dto/                     # org-signup.dto.ts, signin.dto.ts, refresh.dto.ts, candidate-auth.dto.ts
│   ├── candidate-account/
...
| `AuthModule` | `AuthService`, `TenantProvisioningService`, `TokenService` | — |
...
- `ResponseInterceptor` + `ApiExceptionFilter` register in `AppModule` via `APP_INTERCEPTOR` / `APP_FILTER` (consistent with the tenant interceptor and the RolesGuard, and removes the manual re-registration in the e2e spec).
- `TenantContextInterceptor` (`APP_INTERCEPTOR`) and `RolesGuard` (`APP_GUARD`) stay in `AppModule`.
...
1. **`TokenService`** — one `issueTokens(user: { id, tenantId, role })` that signs access + refresh (JWT secrets via `ConfigService`), hashes the refresh token with argon2, upserts via `RefreshTokenRepository`, returns `{ accessToken, refreshToken }`. Replaces the 3 near-duplicate generators. Public `logout(userId)` deletes the stored refresh token via the repo.

2. **`TenantProvisioningService`** — `createTenant(dto)`:
   - checks slug uniqueness (`TenantRepository.findBySlug`),
   - creates the tenant row (`TenantRepository.create`),
   - `CREATE SCHEMA` + clones template tables (single `forSchema('public')` op),
   - seeds OrgAdmin user (`UserRepository.create`, schema `tenant_<id>`),
...
3. **`AuthService`** — orchestration only, no raw Drizzle, no `process.env`, no argon2:
   - `orgSignup(dto)` → `tenantProvisioning.createTenant(dto)` → `tokenService.issueTokens(...)`.
   - `signin(dto)` → resolve email via `userEmails` lookup → tenant schema user → `verifyPassword` → `issueTokens`; fall back to candidate account → superadmin. Returns envelope.
...
**Repo schema scoping:** `forCurrentTenant()` throws outside a tenant context, which killed `UserRepository`. Fix: `BaseRepository` helpers accept an explicit schema name OR fall back to the ALS current-tenant context. Repos that need cross-tenant reads (signin) pass the schema explicitly; tenant-context reads use the default.

## BaseRepository & Consistent Shapes

- `BaseRepository` constructor injects `DrizzleSchemaService`; exposes `withDb(schema, fn)` that acquires a client, sets `search_path`, runs `fn(db)`, and releases in `finally`. Default schema resolution: explicit arg > current-tenant (ALS) > public.
...
- `AuthController`: replace inline body types with `OrgSignupSchema`, `SigninSchema`, `RefreshSchema` (new DTOs), and enforce the existing `CandidateSignupSchema`.
...
- **`CurrentUser` decorator** — reads typed `req.user` (`{ userId, tenantId, role }`); replaces `@Request() req: any` (auth controller) and `getCurrentUser()` ALS import (candidate controller).
...
- `TenantContext` + accessors move to `common/context/tenant-context.ts`; interceptor imports from there.

## Testing

- New/updated unit specs: `token.service.spec.ts`, `tenant-provisioning.service.spec.ts`, `zod-validation.pipe.spec.ts`, updated `auth.service.spec.ts` / `auth.controller.spec.ts` / `candidate-account.controller.spec.ts` (if present).
