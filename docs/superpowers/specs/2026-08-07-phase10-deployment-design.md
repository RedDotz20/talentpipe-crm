...
- **Resume viewing fix (included):** the company candidate profile links `href={fileUrl}` where `fileUrl` is a bare S3 key — a dead relative URL. Add a backend streaming endpoint and blob-fetch on the frontend so reviewers can actually view resumes in prod.
...
- **Migrations:** one-shot `migrate` compose service (postgres:16-alpine image, bind-mounted `backend/drizzle/` + `scripts/prod-migrate.sh`). Idempotent guard: skips when `public.companies` exists. `backend` waits on `service_completed_successfully`.
...
5. `scripts/prod-migrate.sh` — applies `drizzle/*/migration.sql` chronologically + `template-schema.sql` via psql; exits 0 early if `public.companies` exists.
6. Resume view fix:
...
Local before handoff: `docker compose -f docker-compose.prod.yml config` → `build` → full stack up → curl smoke (health, company signup, signin, publish job, public careers, resume upload + view). Then user deploys to the server.
