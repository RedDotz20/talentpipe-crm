...
- Candidates **cannot upload their own resume** — the only resume upload endpoint is company-facing (`POST /candidates/:candidateId/resume`, roles CompanyAdmin/Recruiter)
- The company candidate detail page (`CandidateProfile.tsx`) shows a `<ResumeUploadInput>` when a candidate has no resume — **companies can upload resumes for candidates**
- Apply flow is broken: frontend posts to `/candidate/jobs/${jobId}/apply` but backend expects `/candidate/jobs/:companyId/:jobId/apply`; `ApplyData` includes fields (firstName, lastName, email, coverLetter, resumeUrl) the backend ignores
- Candidate ↔ company candidate link is **by email only** (no UUID FK); changing email would break all company links and the same person applying to multiple companies creates duplicate candidate rows
...
4. **Companies are view-only** — remove company create-candidate and company resume-upload endpoints; companies only see candidates who applied; company candidate view shows read-only profile, resume link, skills, applications
5. **Email is editable safely** — UUID link replaces email link; company `candidates` gets `candidate_account_id` column set on apply
...
### MinIO Storage
- Old key scheme: `companies/{companyId}/resumes/{candidateId}/{uuid}.ext`
- **New key scheme:** `candidate-resumes/{candidateAccountId}/{uuid}.ext` — candidate context has no company, so the S3 key uses the candidate's UUID directly

### Company Schema (company-owned, now a linked view)
...
| `resumes` | **Removed** — company no longer uploads; resume lives publicly |
...
- For dev: existing company schemas must be dropped/re-created or manually altered (documented in bootstrap)
- Template clone on signup will produce clean company schemas going forward
...
| `POST /candidate/jobs/:companyId/:jobId/apply` | **Rewritten**: anchors on JWT `userId` (candidate account UUID). Finds/creates company candidate via `candidate_account_id`. Snapshots identity fields + `skillIds` into application. Computes `matchScore` from applied skills. **Duplicate apply** → 409 "You already applied to this application." |

### Company Module (`/candidates/*`, JWT + CompanyAdmin/Recruiter/HiringManager)

| Endpoint | Change |
|----------|--------|
| `POST /candidates` | **Removed** — companies cannot manually create candidates |
| `POST /candidates/:candidateId/resume` | **Removed** — companies cannot upload resumes |
...
- Company upload logic removed
- MinIO key generation updated for candidate context (uses `candidateAccountId` instead of `getCompanyId()`)
...
| `frontend/src/features/candidate-portal/dashboard/JobSearchPage.tsx` | Apply modal: prefill name/email/phone/skills from `useProfile` (email read-only); phone/skills editable per-application; fix broken apply URL to include `companyId`; submit full snapshot. |
...
| File/Component | Change |
|----------------|--------|
| `frontend/src/features/company/candidates/CandidateProfile.tsx` | Remove `<ResumeUploadInput>`; resume as read-only link; skills as read-only badges; identity read-only |
| `frontend/src/features/company/candidates/CandidateList.tsx` | Read-only (unchanged) |
| `frontend/src/features/company/candidates/hooks/useCandidates.ts` | Remove `useCreateCandidate`, `useUploadResume` |
...
1. Candidate browses jobs (list includes `companyId` from `job_listings_index`)
...
3. Candidate submits → `candidateApi.applyToJob(jobId, data)` sends `{ phone, skillIds, coverLetter }` to `POST /candidate/jobs/:companyId/:jobId/apply`
4. Backend:
   - Resolves candidate account from JWT `userId`
   - In company schema: finds/creates `candidates` row via `candidate_account_id` (UUID), sets `name/email/phone` from profile (or overrides)
...
| No resume on profile | Company sees "No resume" state, read-only |
...
- `candidates.service.spec.ts` (company):
...
| Company Module | `candidates.controller.ts`, `candidates.service.ts` (remove create/upload) |
...
| Frontend Company | `CandidateProfile.tsx`, `useCandidates.ts`, `candidatesApi.ts` |
